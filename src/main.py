...
def main():
    ...
    for line in lineImages:
        print("Segmenting line:",str(i))
        wordsForLine = []
        ws = WordSegmentation(line)
        wordsForLine = ws.segmentIntoWords()
        EntireDocwordImages.append(wordsForLine)
        print("Line:",str(i),"segmented in :",len(wordsForLine),' words')
        i+=1 
    ...
    LINES = []
    for wordsInLines in EntireDocwordImages:
        lineText = ''
        for word in wordsInLines:
            rword = infer(model,word)
            lineText+=rword
            lineText+=' '
        lineText+="\n"
    ...
    if args.train or args.validate:
        ...
        if args.train:
            ...
            Model(loader.charList, decoderType, mustRestore=True)
            validate(model, loader)
    # infer text on test image
    else:
        print(open(FilePaths.fnAccuracy).read())
        model = Model(open(FilePaths.fnCharList).read(), decoderType, mustRestore=True, dump=args.dump)
        testList = ['../data/test.png','../data/test1.png'] 
